...
describe('graphql mutation', () => {
    ...
    it('test article add then update then delete', (done) => {
        fetch('/graphql', {
            ...
            expect(addId).to.not.be.an('undefined');
            return addId
        }).then(addId => {
            fetch('/graphql', {
                method: 'post',
                ...
                body: JSON.stringify({
                    query: `query { books(id:"${addId}") { id,title,content,createTime,author{source,sourceId,displayName} } }`
                }),
                credentials: 'include'
            }).then(resp => {
                expect(resp.status).to.be.equal(200)
                ...
                for (var index in books) {
                    ...
                    expect(books[index].author).to.not.be.a('null');
                    expect(books[index].author.displayName).to.not.be.a('null');
                }
